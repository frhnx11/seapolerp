...
import { deleteDoVessel } from "./do-vessel-actions";
import { formatDoVesselId, type DoVesselRow } from "./do-vessel";

export function DeleteDoVesselModal({
  vessel,
  onClose,
  onDeleted,
}: {
  ...
  async function handleDelete() {
    setLoading(true);
    const result = await deleteDoVessel(vessel.id);
    setLoading(false);
    if (result.ok) {
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p className="mt-2 text-sm text-gray-600">
          ...
          <span className="font-semibold text-gray-900">
            {formatDoVesselId(vessel.seq)} — {vessel.name}
          </span>
